...
pdf_url: "https://www.boe.es/boe/dias/2018/09/20/pdfs/BOE-A-2018-12742.pdf"
subjects: ["Ayuntamientos", "Censos de población", "Instituto Nacional de Estadística", "Padrón Municipal", "Población de las entidades locales"]
...
url_html_consolidada: "https://www.boe.es/buscar/act.php?id=BOE-A-2018-12742"
url_pdf: "https://www.boe.es/boe/dias/2018/09/20/pdfs/BOE-A-2018-12742.pdf"
url_epub: "https://www.boe.es/diario_boe/epub.php?id=BOE-A-2018-12742"
page_start: "90826"
page_end: "90843"
image_marker: "A"
legislative_status: "L"
alerts: "Organización de la Administración"
references_previous: "DEROGA BOE-A-2005-19254; DE CONFORMIDAD con BOE-A-1985-5392"
---
...
| --- | --- | --- | --- | --- | --- |
| Situación en IDA_Padrón a 1 de enero | Situación en IDA_Padrón a 1 de enero | Situación en IDA_Padrón a 1 de enero | Situación en IDA_Padrón a 1 de enero | Situación en IDA_Padrón a 1 de enero | Situación en IDA_Padrón a 1 de enero |
...
| --- | --- |
| NR | No en Reparos<sup>1</sup>. (Desestimada automáticamente). |
| AI | Alegación inconsistente<sup>2</sup>. (Desestimada automáticamente). |
...
| NV | No válida<sup>3</sup>. (Desestimada automáticamente). |
...
| <sup>1</sup> Se ha realizado un contraste entre el registro de alegaciones y el devuelto en el fichero de Reparos. Cualquier cambio en alguno de los campos implica que no se identifiquen, por tanto, con esta clave pueden aparecer alegaciones devueltas en Reparos, para las que se habrá modificado algún campo no incluido en las salvedades previstas. <sup>2</sup> Por ejemplo si se alega con un ANA72 para un nacimiento NO devuelto en Reparos con error 72. <sup>3</sup> Cuando se ha utilizado una CALEG distinta a las contempladas en esta Resolución o en la relación actualizada de IDA-Padrón. | <sup>1</sup> Se ha realizado un contraste entre el registro de alegaciones y el devuelto en el fichero de Reparos. Cualquier cambio en alguno de los campos implica que no se identifiquen, por tanto, con esta clave pueden aparecer alegaciones devueltas en Reparos, para las que se habrá modificado algún campo no incluido en las salvedades previstas. <sup>2</sup> Por ejemplo si se alega con un ANA72 para un nacimiento NO devuelto en Reparos con error 72. <sup>3</sup> Cuando se ha utilizado una CALEG distinta a las contempladas en esta Resolución o en la relación actualizada de IDA-Padrón. |
...
| DM | AC | Alegación revisada manualmente y estimada. |
| NV | NV | Alegación con CALEG no contemplada. |
| nn(<71) | nn(<71) | Alegación desestimada por error invalidante nn. |
| nn(>70) | DS | Alegación desestimada por error de gestión nn<sup>4</sup>. |
| nn(>70) | nn(>70) | Alegación desestimada por error de gestión nn |
| nn(>70) | AC | Alegación revisada automática o manualmente y estimada. |
| nn(>70) | GB | La alegación genera una baja en el mismo u otro municipio donde ya se ha contabilizado (procedimiento automático) |
...
| MA | AC | Alegación que tras intentar su carga se ha ido a Manuales. Revisada automática o manualmente ha sido estimada. |
| MA | GB | La alegación genera una baja en el mismo u otro municipio donde ya se ha contabilizado (procedimiento automático). |
| RA | DS | Alegación revisada manualmente y desestimada porque no procede. |
| RA | AC | Alegación revisada manualmente y estimada. |
| T2 | DS | Alegación que se ha intentado su carga. Revisada automática o manualmente ha sido desestimada. |
| T2 | AC | Alegación que se ha intentado su carga. Revisada automática o manualmente ha sido estimada. |
| T1 | DS | Alegación que no va a carga. Revisada automática o manualmente ha sido desestimada. |
| T1 | AC | Alegación que no va a carga. Revisada automática o manualmente ha sido estimada. |
| BA | BA | Alegación de baja aceptada. |
| <sup>4</sup> En el caso de error 71 porque ya se ha contabilizado en el mismo u otro municipio. | <sup>4</sup> En el caso de error 71 porque ya se ha contabilizado en el mismo u otro municipio. | <sup>4</sup> En el caso de error 71 porque ya se ha contabilizado en el mismo u otro municipio. |
...
| Datos iniciales (INE). | CMUN * | Código de Municipio de Residencia. | N(3) |
| Datos iniciales (INE). | DIST | Distrito. | N(2 |
| Datos iniciales (INE). | SECC | Sección. | N(3) |
| Datos iniciales (INE). | LSECC | Letra de Sección. | A(1) |
| Datos iniciales (INE). | CUN | Código de Unidad Poblacional. | N(7) |
| Datos iniciales (INE). | NENTCOC | Nombre Corto Entidad Colectiva. | A(25) |
| Datos iniciales (INE). | NENTSIC | Nombre Corto Entidad Singular. | A(25) |
| Datos iniciales (INE). | NNUCLEC | Nombre Corto Núcleo/Diseminado. | A(25) |
| Datos iniciales (INE). | CVIA | Código de Vía. | N(5) |
| Datos iniciales (INE). | TVIA | Tipo de Vía. | A(5) |
| Datos iniciales (INE). | NVIAC | Nombre Corto Vía. | A(25) |
| Datos iniciales (INE). | CPSVIA | Código de Pseudovía. | N(5) |
| Datos iniciales (INE). | DPSVIA | Nombre de Pseudovía. | A(50) |
| Datos iniciales (INE). | TINUM | Tipo de Numeración. | N(1) |
| Datos iniciales (INE). | NUMER | Número. | N(4) |
| Datos iniciales (INE). | CNUMER | Calificador de NUMER. | A(1) |
| Datos iniciales (INE). | NUMERS | Número (Superior). | N(4) |
| Datos iniciales (INE). | CNUMERS | Calificador de NUMERS (Superior). | A(1) |
| Datos iniciales (INE). | KMT | Kilómetro. | N(3) |
| Datos iniciales (INE). | HMT | Hectómetro. | N(1) |
| Datos iniciales (INE). | BLOQ | Bloque. | A(2) |
| Datos iniciales (INE). | PORT | Portal. | A(2) |
| Datos iniciales (INE). | ESCA | Escalera. | A(2) |
| Datos iniciales (INE). | PLAN | Planta. | A(3) |
| Datos iniciales (INE). | PUER | Puerta. | A(4) |
| Datos iniciales (INE). | TLOC | Tipo de Local. | A(1) |
| Datos iniciales (INE). | NIA | Número de Identificación del Ayuntamiento. | A(15) |
| Datos iniciales (INE). | NHOP | Número de Hoja Padronal. | A(10) |
| Datos iniciales (INE). | NIE | Núm. Identificación Electoral. | N(11) |
| Datos iniciales (INE). | NOMB | Nombre del Habitante. | A(20) |
| Datos iniciales (INE). | PART1 | Partícula del Primer Apellido. | A(6) |
| Datos iniciales (INE). | APE1 | Primer Apellido. | A(25) |
| Datos iniciales (INE). | PART2 | Partícula del Segundo Apellido. | A(6) |
| Datos iniciales (INE). | APE2 | Segundo Apellido. | A(25) |
| Datos iniciales (INE). | SEXO | Sexo. | N(1) |
| Datos iniciales (INE). | CPRON | Código Provincia de Nacimiento. | N(2) |
| Datos iniciales (INE). | CMUNN | Código de Municipio de Nacimiento. | N(3) |
| Datos iniciales (INE). | FNAC | Fecha de Nacimiento. | N(8) |
| Datos iniciales (INE). | TIDEN | Tipo de Identificador. | N(1) |
| Datos iniciales (INE). | LEXTR | Letra Documento Extranjeros. | A(1) |
| Datos iniciales (INE). | IDEN | Identificador. | N(8) |
| Datos iniciales (INE). | LIDEN | Código de control del Identificador. | A(1) |
| Datos iniciales (INE). | NDOCU | Número de Documento. | A(20) |
| Datos iniciales (INE). | CNES | Código Nivel de Estudios. | N(2) |
| Datos iniciales (INE). | NACI | Código de Nacionalidad. | N(3) |
| Datos iniciales (INE). | PRDP | Código de Provincia de Destino o Procedencia. | N(2) |
| Datos iniciales (INE). | MUDP | Código de Municipio de Destino o Procedencia. | N(3) |
| Datos iniciales (INE). | CODP | Código de Consulado de Destino-Procedencia. | N(3) |
| Datos de control. | TIPOINF | Tipo de Información. | A(1) |
| Datos de control. | CDEV | Causa de Devolución. | A(2) |
| Datos de control. | FVAR | Fecha de Variación. | N(8) |
| Datos de control. | CVAR | Código de Variación. | A(1) |
| Datos de control. | CAUV | Causa de Variación. | A(2) |
| Datos finales (Ayto.) | DIST | Distrito. | N(2 |
| Datos finales (Ayto.) | SECC | Sección. | N(3) |
| Datos finales (Ayto.) | LSECC | Letra de Sección. | A(1) |
| Datos finales (Ayto.) | CUN | Código de Unidad Poblacional. | N(7) |
| Datos finales (Ayto.) | NENTCOC | Nombre Corto Entidad Colectiva. | A(25) |
| Datos finales (Ayto.) | NENTSIC | Nombre Corto Entidad Singular. | A(25) |
| Datos finales (Ayto.) | NNUCLEC | Nombre Corto Núcleo/Diseminado. | A(25) |
| Datos finales (Ayto.) | CVIA | Código de Vía. | N(5) |
| Datos finales (Ayto.) | TVIA | Tipo de Vía. | A(5) |
| Datos finales (Ayto.) | NVIAC | Nombre Corto Vía. | A(25) |
| Datos finales (Ayto.) | CPSVIA | Código de Pseudovía. | N(5) |
| Datos finales (Ayto.) | DPSVIA | Nombre de Pseudovía. | A(50) |
| Datos finales (Ayto.) | TINUM | Tipo de Numeración. | N(1) |
| Datos finales (Ayto.) | NUMER | Número. | N(4) |
| Datos finales (Ayto.) | CNUMER | Calificador de NUMER. | A(1) |
| Datos finales (Ayto.) | NUMERS | Número (Superior). | N(4) |
| Datos finales (Ayto.) | CNUMERS | Calificador de NUMERS (Superior). | A(1) |
| Datos finales (Ayto.) | KMT | Kilómetro. | N(3) |
| Datos finales (Ayto.) | HMT | Hectómetro. | N(1) |
| Datos finales (Ayto.) | BLOQ | Bloque. | A(2) |
| Datos finales (Ayto.) | PORT | Portal. | A(2) |
| Datos finales (Ayto.) | ESCA | Escalera. | A(2) |
| Datos finales (Ayto.) | PLAN | Planta. | A(3) |
| Datos finales (Ayto.) | PUER | Puerta. | A(4) |
| Datos finales (Ayto.) | TLOC | Tipo de Local. | A(1) |
| Datos finales (Ayto.) | NIA | Número de Identificación del Ayuntamiento. | A(15) |
| Datos finales (Ayto.) | NHOP | Número de Hoja Padronal. | A(10) |
| Datos finales (Ayto.) | NIE | Núm. Identificación Electoral. | N(11) |
| Datos finales (Ayto.) | NOMB | Nombre del Habitante. | A(20) |
| Datos finales (Ayto.) | PART1 | Partícula del Primer Apellido. | A(6) |
| Datos finales (Ayto.) | APE1 | Primer Apellido. | A(25) |
| Datos finales (Ayto.) | PART2 | Partícula del Segundo Apellido. | A(6) |
| Datos finales (Ayto.) | APE2 | Segundo Apellido. | A(25) |
| Datos finales (Ayto.) | SEXO | Sexo. | N(1) |
| Datos finales (Ayto.) | CPRON | Código Provincia de Nacimiento. | N(2) |
| Datos finales (Ayto.) | CMUNN | Código de Municipio de Nacimiento. | N(3) |
| Datos finales (Ayto.) | FNAC | Fecha de Nacimiento. | N(8) |
| Datos finales (Ayto.) | TIDEN | Tipo de Identificador. | N(1) |
| Datos finales (Ayto.) | LEXTR | Letra Documento Extranjeros. | A(1) |
| Datos finales (Ayto.) | IDEN | Identificador. | N(8) |
| Datos finales (Ayto.) | LIDEN | Código de control del Identificador. | A(1) |
| Datos finales (Ayto.) | NDOCU | Número de Documento. | A(20) |
| Datos finales (Ayto.) | CNES | Código Nivel de Estudios. | N(2) |
| Datos finales (Ayto.) | NACI | Código de Nacionalidad. | N(3) |
| Datos finales (Ayto.) | PRDP | Código de Provincia de Destino o Procedencia. | N(2) |
| Datos finales (Ayto.) | MUDP | Código de Municipio de Destino o Procedencia. | N(3) |
| Datos finales (Ayto.) | CODP | Código de Consulado de Destino-Procedencia. | N(3) |
...
> <sup>1</sup> La descripción de los errores figura en el documento Normas para el tratamiento de las incidencias resultantes de la incorporación de las variaciones mensuales a los ficheros padronales del INE, que figura en IDA-Padrón (apartado Diseños de registro).
